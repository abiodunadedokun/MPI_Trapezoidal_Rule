/*    MPI Implementation of Trapezoidal Using collective communications to distribute input data and compute the global sum.
 
 * Input:    Number of trapezoids
 * Output:   Estimate of the integral from a to b of function f(x) using the Trapezoidal rule and n trapezoids.
 ...
 *    3a. Each process whose rank != 0 sends its calculations to process 0.
 *    3b. Process 0 sums the calculations received from
 *        the individual processes and prints the result.
 *
 *   f(x) is hardwired to x*x.
 *
 */
#include <stdio.h>
#include <stdlib.h>
...
int main(int argc, char** argv) {
   ...
   double start, stop, loc_elapsed, elapsed;

   MPI_Init(NULL, NULL);
   MPI_Comm_rank(MPI_COMM_WORLD, &my_rank);
   MPI_Comm_size(MPI_COMM_WORLD, &p);

   Get_input(argc, argv, my_rank, &n); /*Reads input */

   /*h and local_n are defined and the same for all processes*/
   h = (b-a)/n;          /* length of each trapezoid */
   local_n = n/p;  /* number of trapezoids per process */
   ...
   start = MPI_Wtime();
   /* Calculates each process' local integral using local endpoints*/
   local_int = Trap(local_a, local_b, local_n, h);
   stop = MPI_Wtime();
   loc_elapsed = stop-start;
   MPI_Reduce(&loc_elapsed, &elapsed, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);

   /* 	MPI_Reduce sums up the integrals calculated by each process */
...
void Get_input(int argc, char* argv[], int my_rank, double* n_p){
	...
	// 	MPI_Broadcast Broadcasts value of n to each process
